...
if uploaded_file is not None:
    ...
    if not (col_time and col_spot and col_fut):
        ...
        date_range = st.sidebar.date_input("选择分析起止时间", value=(min_date, max_date), min_value=min_date, max_value=max_date)
        ...
        if isinstance(date_range, tuple) and len(date_range) == 2:
            ...
            col1.metric("累计补仓资金", f"{df['Cash_Injection'].sum()/10000:.2f} 万", delta_color="inverse")
            col2.metric("累计出金盈余", f"{df['Cash_Withdrawal'].sum()/10000:.2f} 万")
            col3.metric("稳定性提升", f"{stability_boost:.1f}%", help="资产波动率(标准差)降低的百分比")
            col4.metric("风险度均值", f"{df['Risk_Degree'].mean() * 100:.1f}%")

            # --- 图表排序调整：波动性分析在前 ---
            tab1, tab2, tab3, tab4 = st.tabs(["🛡️ 对冲波动稳定性", "📊 风险概率分布", "🏦 资金通道监管", "📉 价格基差监控"])

            with tab1:
                st.subheader("资产价值波动稳定性对比")
                fig4, ax4 = plt.subplots(figsize=(10, 5))
                ax4.plot(df['Date'], val_raw, 'r-', alpha=0.3, label='未套保：现货价值波动')
                ax4.plot(df['Date'], val_hedge, 'g-', linewidth=2, label='套保后：综合资产净变动')
                ...
                # 波动性对比文字
                st.info(f"💡 **稳定性分析**：未套保资产的标准差为 **{std_raw:.2f}**，套保后降低至 **{std_hedge:.2f}**。")
            ...
            with tab3:
                st.subheader(f"资金安全通道与调仓点 (警戒线: {inject_ratio}x | 提盈线: {withdraw_ratio}x)")
                ...
                # --- 新增：体现补仓和出金的点 ---
                inject_events = df[df['Cash_Injection'] > 0]
                withdraw_events = df[df['Cash_Withdrawal'] > 0]
                ax3.scatter(inject_events['Date'], inject_events['Account_Equity']/10000, color='red', marker='^', s=60, label='补仓点', zorder=5)
                ax3.scatter(withdraw_events['Date'], withdraw_events['Account_Equity']/10000, color='blue', marker='v', s=60, label='出金点', zorder=5)
            ...
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                ...
                out_df.to_excel(writer, index=False, sheet_name='稳定性回测日报')
            st.download_button(label="📥 下载 Excel 回测数据", data=output.getvalue(), file_name='稳定性回测报告.xlsx')
        else:
            st.warning("请在左侧侧边栏选择完整的开始和结束日期。")
else:
    st.info("👆 请在左侧侧边栏上传 CSV 文件以开启套保稳定性回测。")
